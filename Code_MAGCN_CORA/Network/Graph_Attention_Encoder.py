...
class GATE():
    ...
    def __call__(self, A, X, R, S, p, A2, X2, R2, S2):
        ...
        structure_loss = -tf.log(tf.sigmoid(tf.reduce_sum(self.S_emb * self.R_emb, axis=-1)))\
                         -tf.log(tf.sigmoid(tf.reduce_sum(self.S_emb2 * self.R_emb2, axis=-1)))
        structure_loss = tf.reduce_sum(structure_loss)
        # Total loss
        self.loss = features_loss + \
            self.lambda_ * structure_loss + 10 * self.C_loss + self.dense_loss
        return self.loss, self.H, self.C, self.H2, self.C2, self.pred, \
            self.dense_loss, self.z, self.z2, features_loss, structure_loss
    ...
    def _soft_assignment(self, embeddings, cluster_centers):
        ...
        Return:
            ...
            这段代码是一个 _soft_assignment 方法，用于进行软分配（softmax）操作。它接收两个参数 embeddings 和 cluster_centers，
            分别代表嵌入向量和聚类中心。
        首先，代码定义了一个内部函数 _pairwise_euclidean_distance，它是我们之前讨论过的计算欧氏距离的函数。
        这个函数用于计算 embeddings 和 cluster_centers 之间的欧氏距离。
        接下来，代码通过调用 _pairwise_euclidean_distance 方法计算得到距离矩阵 dist，它表示每个嵌入向量与每个聚类中心之间的欧氏距离。
        然后，代码使用公式进行软分配操作。首先，计算 dist 的平方，除以一个参数 self.alpha 再加上 1，
        再除以 2 的 (self.alpha + 1) / 2.0 次方。这里使用了 TensorFlow 的元素级操作和运算符重载，使得计算可以逐个元素地进行。
        接着，对结果进行归一化，通过除以矩阵 q 的每行和。这样做可以得到一个概率分布，使得每个嵌入向量与聚类中心之间的软分配概率和为 1。
        最后，函数返回计算得到的软分配矩阵 q。
        """
