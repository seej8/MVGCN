...
def load_data(dataset_str): # {'pubmed', 'citeseer', 'cora'}
    ...
    """这段代码的作用是针对citeseer数据集进行处理。在citeseer数据集中存在一些孤立的节点（即没有边连接到它们的节点），
    这段代码的目的是找到这些孤立节点，并将它们添加为零向量（zero-vecs）放置在正确的位置上。
首先，确定了包含测试节点索引的范围test_idx_range_full，这个范围是通过找到test_idx_reorder列表中的最小值和最大值得到的。
然后，创建了一个大小为(len(test_idx_range_full), x.shape[1])的稀疏矩阵tx_extended，用于存储扩展后的特征向量。
接下来，将原始的特征矩阵tx复制到扩展后的稀疏矩阵tx_extended的相应位置。具体来说，使用test_idx_range-min(test_idx_range)
作为行索引和:作为列索引来选择要替换的位置，然后将原始特征矩阵的值复制到这些位置上。
最后，创建了一个大小为(len(test_idx_range_full), y.shape[1])的零矩阵ty_extended，用于存储扩展后的标签向量。
同样地，将原始的标签矩阵ty复制到相应位置。这么做是为了确保标签矩阵的维度与特征矩阵维度相匹配。
总的来说，这段代码的目的是通过扩展特征矩阵和标签矩阵，解决citeseer数据集中的孤立节点问题，使得特征矩阵和标签矩阵的维度对应正确。"""
